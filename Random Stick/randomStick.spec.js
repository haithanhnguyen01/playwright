...
test("Tidal design with multiple variants", async ({ page }) => {
    await page.goto('https://www.lodes.com/en/products/random-stick/?code=eu', { waitUntil: 'domcontentloaded' });
    ...
    for (let variantIndex = 0; variantIndex < count; variantIndex++) {
        ...
        if (await body.isVisible()) {
            ...
            if (variantIndex == 0) {
                for (let i = 0; i < 6; i++) {
                    let sizes = await body.locator('.value-table-di').nth(i).innerText(); // get the first word of the product name
                    let size = sizes.split('\n')[1];
                    SphereArray.push(size);
                };
            } else {
                for (let i = 0; i < 5; i++) {
                    let sizes = await body.locator('.value-table-di').nth(i).innerText(); // get the first word of the product name
            ...
            const ColorArray = [];
            const BlowglassDiffuser = await page.locator('td.icons').nth(1);
            const colorImgs = await BlowglassDiffuser.locator('img');
            

            for (let i = 0; i < 7; i++) {
                const color = await colorImgs.nth(i).getAttribute('alt');
            ...
            fullProducts.push({
                ...
                SizeSphere: SphereArray,
                BlownGlassDiffuser: ColorArray,
            });
        }
    }
