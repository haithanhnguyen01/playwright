...
test("Tidal design with multiple variants", async ({ page }) => {
    await page.goto('https://www.lodes.com/en/products/urban-concrete/?code=eu', { waitUntil: 'domcontentloaded' });
    ...
    for (let variantIndex = 0; variantIndex < count; variantIndex++) {
        ...
        if (await body.isVisible()) {
            ...
            const name = productName.split(' ')[2];
            let productDetails = [];
            ...
            const formatColorForURL = (colorName) => {
                let name = colorName.replace(/\s+/g, '-'); 
                return name
                    .replace(/é/g, 'e')
                    .replace(/[^\w-]/g, ''); 
            };
            ...
            if (table) {
                ...
                for (const row of rows) {
                    ...
                    for (let i = 0; i < rowCount; i++) {
                        ...
                        const canopyImg = await canopyRows[i].$('td img');
                        ...
                        const ColorUrl = `https://www.lodes.com/wp-content/uploads/2021/03/Urban-Concrete-${name}-${colorProduct}.png`;

                        productDetails.push({
                            Code: codeText,
                            InsideDiffuser: structureAlt,
                            OutsideDiffuser: canopyAlt,
                            Lamp: parsedLamp2700,
                    ...
                    //     // const colorProduct = formatColorForURL(canopyAlt);
                    //     const ColorUrl = `https://www.lodes.com/wp-content/uploads/2023/07/UFO-${name}-${canopyAlt}.png`;
