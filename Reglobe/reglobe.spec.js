...
test("Tidal design with multiple variants", async ({ page }) => {
    await page.goto('https://www.lodes.com/en/products/reglobe/?code=eu', { waitUntil: 'domcontentloaded' });
    ...
    for (let variantIndex = 0; variantIndex < count; variantIndex++) {
        ...
        if (await body.isVisible()) {
            ...
            const moreInfo = await page.locator('div.nota.marginb20.font11').nth(variantIndex).innerText();
            ...
            if (table) {
                ...
                for (const row of rows) {
                    ...
                    for (let i = 0; i < rowCount; i++) {
                        ...
                        const ColorUrl = `https://www.lodes.com/wp-content/uploads/2023/07/Reglobe-${type}.png`;
                        const Dimm = await body.locator('a.bottone.no-hover.marchio img').nth(3).getAttribute('alt');
