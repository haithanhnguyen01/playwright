...
test("Tidal design with multiple variants", async ({ page }) => {
  await page.goto('https://www.lodes.com/en/products/sky-fall-round/?code=eu', { waitUntil: 'domcontentloaded' });
  ...
  for (let variantIndex = 0; variantIndex < count; variantIndex++) {
    ...
    if (await body.isVisible()) {
      ...
      let type = productName.split(' ')[2]; // get the last word of the product name

      if(variantIndex == 0) {
        type = type.charAt(0).toUpperCase() + type.slice(1).toLowerCase(); // capitalize the first letter
      }
      ...
      if (table) {
        ...
        if (variantIndex == 2 || variantIndex == 3) {
          if (variantIndex == 2) {
            const System = await page.locator('td.scheda-tecnica-td').nth(variantIndex).innerText();
            const codeText = await body.locator('td.codes table tr td').innerText();

            const structureAlt = await body.locator('td.icons img').getAttribute('alt');
            productDetails.push({
              Code: codeText,
              System: System || 'N/A',
          ...
          for (const row of rows) {
            ...
            for (let i = 0; i < rowCount; i++) {
              ...
              const ColorUrl = `https://www.lodes.com/wp-content/uploads/2024/01/Sky-Fall-Round-${type}-${colorProduct}.png`;
